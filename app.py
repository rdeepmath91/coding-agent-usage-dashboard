...
Reads local usage data from OpenCode, Codex CLI, and Hermes sources and serves a dark-themed web UI.
"""
...
def hermes_records(days: int | None = None) -> list[dict]:
    ...
    """
    if not hermes_source_available():
        return []
    ...
    conn = sqlite3.connect(f"file:{HERMES_STATE_PATH}?mode=ro", uri=True)
    conn.row_factory = sqlite3.Row
    try:
        rows = conn.execute(f"""
            SELECT
    ...
    for row in rows:
        ...
        records.append({
            "tool": "Hermes",
            "tool_id": "hermes",
            "tool_color": TOOL_COLOR_MAP.get("hermes", "#EAB308"),
            "source_path": display_path(HERMES_STATE_PATH),
            "id": row["id"],
            "session_id": row["id"],
